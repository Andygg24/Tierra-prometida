...
const rowToRendimiento = (r) => ({
  ...
  kilosRechazados: Number(r.kilos_rechazados) || 0,
...
export function useContenedores() {
  ...
  const guardarRendimiento = useCallback(async (form, editId = null) => {
    ...
    const row = {
      ...
      kilos_rechazados: Number(form.kilosRechazados)  || 0,
      kilos_devueltos:  (Number(form.kilosRechazados) || 0) + (Number(form.kilosPrimeraDevueltos) || 0),
