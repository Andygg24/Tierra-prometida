...
export default function SearchableSelect({ value, onChange, children, style, disabled, placeholder = "Buscar..." }) {
  ...
  useEffect(() => {
    if (!open) { setQuery(""); setHoveredIdx(-1); return; }
  ...
  const handleInputKeyDown = (e) => {
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setHoveredIdx(i => Math.min(filteredOptions.length - 1, i + 1));
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      setHoveredIdx(i => Math.max(0, i - 1));
    } else if (e.key === "Enter") {
      e.preventDefault();
      const opt = filteredOptions[hoveredIdx] ?? filteredOptions[0];
  ...
  const dropdown = open ? createPortal(
    <div
      ...
      style={{
        ...
        `,
        animation:            "ssOpen 0.16s cubic-bezier(0.16,1,0.3,1)",
        overflow:             "hidden",
      }}
    >
      <div style={{ padding: "6px 6px 4px", borderBottom: "1px solid rgba(255,255,255,0.08)", flexShrink: 0 }}>
        <input
          ref={inputRef}
          value={query}
      ...
      >
        {filteredOptions.length === 0 && (
          <div style={{ padding: "12px 14px", fontSize: 11, color: "rgba(255,255,255,0.4)", textAlign: "center" }}>
            Sin resultados
          </div>
        )}
